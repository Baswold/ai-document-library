"""Semantic search utilities for the AI Document Library.

This module gracefully degrades when optional semantic-search dependencies are
missing.  When ``sentence-transformers`` or ``faiss`` are unavailable the
library continues to operate using keyword-only search so the main application
remains functional in lightweight environments.
"""
...
class SemanticSearchEngine:
    def __init__(self, db_path: str, model_name: str = "all-MiniLM-L6-v2"):
        ...
        self.semantic_enabled = bool(
            np is not None and SentenceTransformer is not None and faiss is not None
        )

        if not self.semantic_enabled:
            self.logger.warning(
                "Semantic search dependencies are unavailable. Falling back to "
                "keyword search only. Install 'sentence-transformers', 'faiss-cpu' "
                "and 'numpy' to enable semantic search."
            )
    ...
    def generate_embedding(self, text: str):
        ...
        if not self.semantic_enabled:
            raise RuntimeError(
                "Semantic search is disabled – embeddings are unavailable."
            )
    ...
    def semantic_search(self, query: str, top_k: int = 5) -> List[Dict]:
        ...
        similarities, indices = self.index.search(
            query_vector, min(top_k, len(self.chunk_ids))
        )
    ...
    def store_search_analytics(self, query: str, results_count: int, similarities: Sequence):
        ...
        try:
            if np is not None and hasattr(similarities, "size"):
                ...
                similarities_list = list(similarities or [])
                avg_similarity = (
                    sum(float(value) for value in similarities_list) / len(similarities_list)
